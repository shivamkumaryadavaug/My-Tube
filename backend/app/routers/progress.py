...
def today_stats(current_user: models.User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    sessions_today = (
        ...
        .filter(
            ...
            models.FocusSession.created_at >= day_start,
            models.FocusSession.created_at < day_end,
        )
        .all()
    )
    videos_today = (
        ...
        .filter(
            ...
            models.Video.completed.is_(True),
            models.Video.completed_at >= day_start,
            models.Video.completed_at < day_end,
        )
        .count()
    )
    return schemas.TodayStats(
        minutes=sum(s.minutes for s in sessions_today),
